...
public class DebtPaymentHistoryController {
    ...
    @GetMapping("/all/by-owner/filter")
    @PreAuthorize("hasAnyAuthority('STAFF','STORE_OWNER')")
    public ResponseEntity<?> getAllOwnerTransactionHistoryByCustomerAndFilter(@RequestParam Long customerId, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startDate,
                                                                         @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endDate) {
        try {
            List<DebtPaymentResponse> transactions = service.getAllOwnerTransactionHistoryByCustomerAndDateRange(customerId, startDate, endDate);
            return ResponseEntity.ok().body(transactions);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        } catch (Exception e) {
    ...
    @PreAuthorize("hasAnyAuthority('STAFF','STORE_OWNER')")
    public ResponseEntity<?> createOwnerDebtPaymentHistory(@RequestBody OwnerDebtPaymentHistoryReq req) {
        try {
            DebtPaymentResponse res = service.saveOwnerDebtPaymentHistory(req);
            return ResponseEntity.ok().body(res);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An error occurred while create owner's transaction!");
        }
    }
}
